...
class Tracker:
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        # Process frames in batches for massive speedup on MPS/GPU
        batch_size = 16
        batch = []
        frame_num = 0
        ...
        # Read generator and fill batches
        for frame in frames:
            batch.append(frame)
            if len(batch) == batch_size:
                process_batch(batch, frame_num)
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        overlay = frame.copy()
        cv2.rectangle(overlay, (10, 120), (500, 240), (255,255,255), -1 )
        alpha = 0.4
        ...
        cv2.putText(frame, f"Team 1 Ball Control: {team_1*100:.2f}%", (20,160), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3)
        cv2.putText(frame, f"Team 2 Ball Control: {team_2*100:.2f}%", (20,210), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 3)

        return frame

    def draw_annotations(self, video_frames_generator, tracks, team_ball_control):
        """
        Yields annotated frames one by one (streaming) to avoid loading the whole video in RAM.
        """
        for frame_num, frame in enumerate(video_frames_generator):
            frame = frame.copy()
            ...
            if team_ball_control is not None and len(team_ball_control) > frame_num:
